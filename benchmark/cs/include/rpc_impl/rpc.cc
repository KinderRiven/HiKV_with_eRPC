    ...
    , req_func_arr_(nexus->req_func_arr_)
{
    ...
    if (ERPC_LOG_LEVEL >= ERPC_LOG_LEVEL_REORDER) {
        const auto trace_filename = "/tmp/erpc_trace_" + std::to_string(nexus->sm_udp_port_) + "-rpc_" + std::to_string(rpc_id);
    ...
    transport_ = new TTr(nexus->sm_udp_port_, rpc_id, phy_port, numa_node_, trace_file_);

    huge_alloc_ = new HugeAlloc(kInitialHugeAllocSize, numa_node_, transport_->reg_mr_func_,
    ...
    for (MsgBuffer& ctrl_msgbuf : ctrl_msgbufs_) {
        ...
        if (ctrl_msgbuf.buf_ == nullptr) {
            ...
            throw std::runtime_error(
                std::string("Failed to allocate control msgbufs. ") + HugeAlloc::kAllocFailHelpStr);
        }
    }
    ...
    if (kCcPacing)
        wheel_->catchup(); // Wheel could be lagging, so catch up
}
